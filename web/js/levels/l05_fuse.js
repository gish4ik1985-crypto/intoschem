...
const Level05 = createLevel(LevelRegistry.register({
  ...
  brief: 'Предохранитель сгорает сразу после включения. На шине три ветки, и одна из них замкнута накоротко — но какая, снаружи не видно.',
  lesson: 'Короткое замыкание — это путь от плюса к минусу почти без сопротивления: ток ограничивать нечем, и он вырастает в сотни раз. Найти такую ветку можно только отключением по одной. Предохранитель при этом не «чинят» перемычкой — он и есть то, что сгорает вместо всего остального.',
  hints: [
  ...
  goalText: 'Обе исправные ветки должны работать, а предохранитель — остаться целым.',
