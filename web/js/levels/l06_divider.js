...
const Level06 = createLevel(LevelRegistry.register({
  ...
  brief: 'Датчику нужны 3,6 В, а в приборе есть только девять. Отдельного источника на 3,6 В не существует — напряжение придётся получить из тех девяти, что есть.',
  lesson: 'Два резистора подряд делят напряжение пропорционально своим сопротивлениям: ток через них общий, значит на большем падает больше. Отношение задаёт, сколько вольт получится. А сумма номиналов задаёт, сколько тока при этом всё время течёт мимо задачи, просто грея воздух.',
  hints: [
    'Нужно 3,6 из 9 — это две пятых. Значит нижний резистор должен быть вдвое меньше верхнего… точнее, отношение 3 к 2.',
  ...
  goalText: 'На датчике 3,4–3,8 В, и делитель тратит меньше 40 мА.',
