...
(() => {
  ...
  createLevel(LevelRegistry.register({
    ...
    brief: 'В ящике три резистора. Поставь по очереди каждый и смотри на ток.',
    lesson: 'Резистор мешает току, а цифра на нём говорит, насколько сильно. Больше ом — меньше ток.',
    ...
    status(m) {
      ...
      if (m.ok) return ['В самый раз. Этот резистор и нужен.', 'good'];
      if (m.i > 0.11) return ['Тока слишком много: этот резистор мешает слишком слабо. Возьми резистор побольше.', 'warn'];
      return ['Тока слишком мало: этот резистор мешает слишком сильно. Возьми резистор поменьше.', 'warn'];
    },
