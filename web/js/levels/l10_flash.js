...
const Level10 = createLevel(LevelRegistry.register({
  ...
  brief: 'Вспышка срабатывает еле-еле. Источник тут ни при чём: он в принципе не способен выдать нужный ток мгновенно — за это отвечает конденсатор, и его надо успеть накопить.',
  lesson: 'Конденсатор набирается не мгновенно: сопротивление, через которое он заряжается, и его собственная ёмкость вместе задают время. Зато отдаёт он всё разом — ток в момент разряда в десятки раз больше того, что способен дать источник.',
  hints: [
  ...
  goalText: 'Дать вспышку с током не меньше 1,2 А.',
