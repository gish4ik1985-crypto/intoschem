...
const Level13 = createLevel(LevelRegistry.register({
  ...
  brief: 'Капля воды на датчике, а лампочка не горит. Через воду ток слишком слабый.',
  lesson: 'Транзистор — кран, который открывает слабый ток. Капля даёт крошечный ток, а транзистор включает им ток в сто раз больше.',
  hints: [
  ...
  goalText: 'Датчик в воде — индикатор горит.',
  ...
  status(m) {
    ...
    if (m.iLed < 0.006) return ['Транзистор на месте, но открыт слабо: перед ним лишний резистор.', 'warn'];
