...
const Level01 = createLevel(LevelRegistry.register({
  ...
  brief: 'Ночное зрение не работает: светодиод не горит. Гнездо между батареями и светодиодом пустое — реши сам, чем его заполнить.',
  lesson: 'Светодиод сам ток не ограничивает: сколько ему дадут, столько он и возьмёт — и сгорит. Ограничивает резистор. Идеального номинала в ящике нет, ближайший разумный — компромисс, и это нормально.',
  hints: [
    ...
    'Нужно 20 мА. На светодиоде падает около 1,8 В, значит на резисторе — остальные 1,2 В. Раздели 1,2 В на 0,02 А.',
  ],
  ...
  goalText: 'Светодиод должен гореть ровно: ток 15–26 мА и не выше.',
  ...
  status(m, P) {
    ...
    if (m.i < 0.015) return ['Горит тускло. В ящике есть номинал поменьше.', 'warn'];
    return ['Горит ровно. Ток в норме — этот номинал годится.', 'good'];
  },
