...
### Для кого `[Зафиксировано]`

**Игрок — ребёнок 10–13 лет.** Игра учит электричеству, и она обязана быть увлекательной. Всё остальное (тексты, цели уровней, сложность, число уровней) проверяется этой строкой.

Что из этого следует, как мерило:

- **Задача уровня** — до 15 слов, **урок** — до 25. Одна мысль на уровень.
- **Слова:** на детской дороге без «потенциала», «милливольт», «номинала», «балласта» и т. п., пока игра сама их не объяснила простыми словами.
- **Цель** — видимый результат («лампа горит ярко и не сгорела», «мотор крутится»), а не попадание числом в допуск. Числа остаются на приборах для любопытных.
- **Физика не упрощается.** Упрощается то, о чём игра спрашивает, и язык, которым она говорит.
- Взрослые и дотошные уровни не удаляются: это «инженерный режим» после детской дороги.

**Детская дорога (утверждена 23.09.2026), 20 уровней по порядку:**
Т-01 кольцо (`t_loop`) → выключатель (`flashlight`) → больше сопротивление, меньше ток (`t_resist`) → закон Ома (`panel`) → гирлянда (`garland`) → **Верстак (`workbench`, открывается сразу после гирлянды как песочница)** → подогрев стекла (`heater`) → защита (`fuse_box`) → мощность (`t_power`) → диод (`t_diode`) → светодиод (`led`) → мотор (`motor`) → делитель (`divider`) → подстройка накала (`v_trim`) → «И» (`g_and`) → «ИЛИ» (`g_or`) → вспышка (`flash`) → маячок (`beacon`) → реле (`pump`) → датчик протечки (`sensor`).
